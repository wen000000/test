...
return      : true  - 成功
********************************************************/
static bool MCP2515_WriteReg(u8 reg,u8 data)
{
...
return      : true - 成功
******************************************************************************/
static bool MCP2515_BitModify(u8 reg,u8 BitSel,u8 BitSta)
{ 
    MCP2515_CS(0);
...
return      : true - 成功
***************************************************************************/
static bool MCP2515_WriteReg_Seq(u8 reg,u8 *data,u8 length)
{
    u8 i=0;
...
return      : true - 成功
***************************************************************************/
static bool MCP2515_ReadReg_Seq(u8 reg,u8 *data,u8 length)
{
    u8 i=0;
    if( NULL == data) return false;
    MCP2515_CS(0);
...
_MCP2515_CAN_MSG *MCP_Can_Recv_Msg(void)
{
    ...
    while( (MCP2515_ReadReg(REG_CANINTF) &  0x01) == 0x01)/* 等待BUFF0报文接收完成 */
    {
        ...
        MCP2515_BitModify(REG_CANINTF,REG_BIT0,BIT_RESET);  /* 清楚BUFF0接收 标志位 */
    }
